...
def build_model(tparams, options):
	...
	y_pred=T.argmax(results, axis=1)
	...
	else: return x, mask, iVector, jVector, total_cost, y_pred
...
def load_data(xFile, dFile, yFile):
	...
	np.savetxt('inputX.txt',seqX,fmt='%s')
	seqD = []
	if len(dFile) > 0: seqD = np.asarray(pickle.load(open(dFile, 'rb')), dtype=config.floatX)
	seqY = []
	if len(yFile) > 0: 
		seqY = np.array(pickle.load(open(yFile, 'rb')))
		np.savetxt('labelY.txt',seqY,fmt='%s')
	...
	if numYcodes > 0:
		...
		for idx, seq in enumerate(seqs):
			if not seq[0] == -1:
				...
				pickTwo(seq, iVector, jVector)
				mask[idx] = 1.	
		np.savetxt('x.txt',x.astype(np.int),fmt='%s')
		np.savetxt('iVector.txt',iVector,fmt='%s')
		np.savetxt('jVector.txt',jVector,fmt='%s')
		np.savetxt('mask.txt',mask,fmt='%s')
...
def train_med2vec(seqFile='seqFile.txt',
	...
	if demoSize > 0 and numYcodes > 0:
		...
		x, mask, iVector, jVector, cost, results = build_model(tparams, options)
		grads = T.grad(cost, wrt=list(tparams.values()))
